...
try:
    ...
    raise ImportError("Cryptography package is not installed. Please install it using 'pip install cryptography'.")
...
async def send_snmp_trap(oids):
    ...
    engine_id = OctetString(hexValue='8000000001020304')
    # Configure SNMPv3 settings
    ...
    # Optionally, to debug, you could print the entire auth_data (be cautious with sensitive information)
    logger.debug(f"auth_data: {auth_data}")

# Log the credentials without logging sensitive data
    logger.debug(f"Authentication Key: {auth_data.authentication_key}")
    logger.debug(f"Privacy Key: {auth_data.privacy_key}")
    logger.debug(f"Authentication Protocol: {auth_data.authentication_protocol}")
    logger.debug(f"Privacy Protocol: {auth_data.privacy_protocol}")
    ...
    try:
        ...
        for communityData, transportTarget, contextData in TARGETS:
            ...
            logger.debug(f"SNMP trap sent using communityData: {communityData}")
...
def api_send_snmp_trap():
    try:
        ...
        try:
            loop.run_until_complete(send_snmp_trap(oids))
            loop.run_until_complete(loop.shutdown_asyncgens())  # Important!
        finally:
            loop.close()
